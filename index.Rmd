...
afro_baro <- afro_baro |>
  select(EA_FAC_D, EA_FAC_G, EA_FAC_F, EA_SEC_C, EA_ROAD_B, Q8A, Q8C, Q8E)
head(afro_baro)
```

## First data visualization & analysis
...
clinic_vs_med <- afro_baro |>
  ...
  summarize(
    mean_no_med = mean(no_med, na.rm = TRUE)
  ) 
knitr::kable(clinic_vs_med, col.names = c("Is there a Health Clinic?", "Average Number of Times Gone Without Treatment"))

clinic_vs_med |>
  ggplot(mapping = aes(x = is_health_clinic, y = mean_no_med)) + geom_col(binwidth = 1) + labs(x = "Is there a Health Clinic Within Walking Distance?", y = "Average Tendency of Going Without Medical Treatment")
```

When analyzing the difference between the average number of times that someone or their family member has gone without treatment based on the existence of a health clinic nearby, the results suggest that the existence of a health clinic nearby affects the ability to acquire medical treatment. When there has been a health clinic or hospital nearby, the average number of times someone or their family member has gone without medical treatment is 0.9486404. However, when there has not been a health clinic or hospital within walking distance, the average number of times someone or their family member has gone without medical treatment tends to increase to 1.1455005. 
